...
public:
	...
	mm_x4_graph() {
		...
		for (int row =0; row<NUM_ENGINES_PER_PAC; row++)  {
			...
			adf::connect<pktstream, window<h1*w1*2>> (sp_a0.out[row], mm_x4[row].in[0]);
			adf::connect<pktstream, window<w1*w2*2>> (sp_b0.out[row], mm_x4[row].in[1]);

			if(row<NUM_ENGINES_PER_PAC-1){
				adf::connect<window<h1*w2*2>> (mm_x4[row].out[0], mm_x4[row+1].in[2]);
			}
			else{
				adf::connect<window<h1*w2*2>>(mm_x4[row].out[0], out);
			}
		}
