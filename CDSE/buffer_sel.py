...
def buff_count_0(BRAM,URAM,PART_A,PART_B,PART_C,LEFT_SIZE,RIGHT_SIZE,OUT_SIZE,a,b,c,x,y,z,left_buf,right_buf,out_buf,RAM_TYPE_A,RAM_TYPE_B,RAM_TYPE_C):
    if RAM_TYPE_A==1:
        bank_Lb=(a*b)*(PART_A)*math.ceil(128/72)*math.ceil(x*y*LEFT_SIZE*2/PART_A/512)
        bank_Lu=(a*b)*(PART_A)*math.ceil(128/72)*math.ceil(x*y*LEFT_SIZE*2/PART_A/4096)
    elif RAM_TYPE_A==2:
        bank_Lb=(a*b)*(PART_A)*math.ceil(128/36)*math.ceil(x*y*LEFT_SIZE/PART_A/1024)
        bank_Lu=(a*b)*(PART_A)*math.ceil(128/72)*math.ceil(x*y*LEFT_SIZE/PART_A/4096)

    if RAM_TYPE_B==1:
        bank_Rb=(c*b)*(PART_B)*math.ceil(128/72)*math.ceil(y*z*RIGHT_SIZE/PART_B/512)
        bank_Ru=(c*b)*(PART_B)*math.ceil(128/72)*math.ceil(y*z*RIGHT_SIZE/PART_B/4096)
    elif RAM_TYPE_B==2:
        bank_Rb=(c*b)*(PART_B)*math.ceil(128/36)*math.ceil(y*z*RIGHT_SIZE/PART_B/1024)
        bank_Ru=(c*b)*(PART_B)*math.ceil(128/72)*math.ceil(y*z*RIGHT_SIZE/PART_B/4096)

    if RAM_TYPE_C==1:
        bank_Ob=(a*c)*(PART_C)*math.ceil(128/72)*math.ceil(x*z*OUT_SIZE/PART_C/512)
        bank_Ou=(a*c)*(PART_C)*math.ceil(128/72)*math.ceil(x*z*OUT_SIZE/PART_C/4096)
    elif RAM_TYPE_C==2:
        bank_Ob=(a*c)*(PART_C)*math.ceil(128/36)*math.ceil(x*z*OUT_SIZE/PART_C/1024)
        bank_Ou=(a*c)*(PART_C)*math.ceil(128/72)*math.ceil(x*z*OUT_SIZE/PART_C/4096)
