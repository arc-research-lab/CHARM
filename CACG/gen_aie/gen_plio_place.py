...
def find_col(col_sel,chl_cnt_in,reverse,num_channel):
    """This is a recursive function
    to find the available PLIO ports"""
    if col_sel>44 or col_sel<6:
        return 0,0
    cnt_pos=col_sel-6
    chl_num=chl_cnt_in[cnt_pos]
    if chl_num < num_channel:
        return col_sel,chl_num
    else:
        col_sel=col_sel+reverse
        return (find_col(col_sel,chl_cnt_in,reverse,num_channel))
    ...
    for i in range(A):
        for j in range(C//A_BRO):
            for k in range((B//PACK_IN)):
                ...
                col_sel_new,chl_num=find_col(col_sel,chl_cnt_in,reverse,num_chl_in)
                if col_sel_new==0:
                    reverse=-reverse
                    col_sel_new,chl_num=find_col(col_sel,chl_cnt_in,reverse,num_chl_in)
                    if col_sel_new==0:
                        print('LHS PLIO failed to place')
                        break
                ...
                chl_cnt_in[col_sel_new-6]=chl_cnt_in[col_sel_new-6]+2
            ...
            for k in range((B//PACK_IN)):
                ...
                if c_a_bro%2==0 and height%2==0:
                    ...
                    if ((j+pos_row)%2)==0:
                        ...
                        reverse=-1
                        offset=1
                cur_col=pos_col+k*PACK_IN+i*B*(C//height)*C_BRO+offset*(B*(C//height)*C_BRO-1)
                col_sel=np.clip(cur_col, 6, 44).astype(int)#PLIOs lie in col 6-44
                col_sel_new,chl_num=find_col(col_sel,chl_cnt_in,reverse,num_chl_in)
                if col_sel_new==0:
                    reverse=-reverse
    ...
    if  height%PACK_OUT==0:
        for i in range(A):
            for j in range(C//PACK_OUT):
                ...
                col_sel=np.clip(cur_col, 6, 44).astype(int)#PLIOs lie in col 6-44
                col_sel_new,chl_num=find_col(col_sel,chl_cnt_out,reverse,num_chl_out)
                if col_sel_new==0:
                    reverse=-reverse
                    col_sel_new,chl_num=find_col(col_sel,chl_cnt_out,reverse,num_chl_out)
                    if col_sel_new==0:
