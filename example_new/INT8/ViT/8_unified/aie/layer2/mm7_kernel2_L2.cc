...
#include <stdio.h>
#include "para_L2.h"
void mm7_kernel2_L2(input_window_int8* __restrict matA, 
    ...
    window_incr(matB,L2_jumpB0);

    buf_matA0=upd_w(buf_matA0,1,window_read_v32(matA));
    window_incr(matA,L2_jumpA0);

    for (int i=0;i<L2_boundary_i;i++)
    chess_prepare_for_pipelining
    chess_loop_range(L2_boundary_i,)
    {
        for (int j=0;j<L2_boundary_j;j++)
        chess_prepare_for_pipelining
