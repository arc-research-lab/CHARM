...
class mm_k1_B2_L3: public graph {
  private:
  ...
  public:
    ...
    mm_k1_B2_L3() {
      ...
      for (int i=0; i<L3_NUM_KERNEL;i++){
        if(i==0){
          mm[i] = kernel::create(mm7_kernel0_L3);
          source(mm[i]) = "mm7_kernel0_L3.cc";
        }
        else if(i==L3_NUM_KERNEL-1){
          mm[i] = kernel::create(mm7_kernel2_L3);
          source(mm[i]) = "mm7_kernel2_L3.cc";
        }
