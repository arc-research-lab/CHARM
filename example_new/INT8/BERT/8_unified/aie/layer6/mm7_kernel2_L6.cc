...
#include <stdio.h>
#include "para_L6.h"
void mm7_kernel2_L6(input_window_int8* __restrict matA, 
    ...
    window_incr(matB,L6_jumpB0);

    buf_matA0=upd_w(buf_matA0,1,window_read_v32(matA));
    window_incr(matA,L6_jumpA0);

    for (int i=0;i<L6_boundary_i;i++)
    chess_prepare_for_pipelining
    chess_loop_range(L6_boundary_i,)
    {
        for (int j=0;j<L6_boundary_j;j++)
        chess_prepare_for_pipelining
