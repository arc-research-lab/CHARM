...
class mm_k1_B2_L2: public graph {
  private:
  ...
  public:
    ...
    mm_k1_B2_L2() {
      ...
      for (int i=0; i<L2_NUM_KERNEL;i++){
        if(i==0){
          mm[i] = kernel::create(mm7_kernel0_L2);
          source(mm[i]) = "mm7_kernel0_L2.cc";
        }
        else if(i==L2_NUM_KERNEL-1){
          mm[i] = kernel::create(mm7_kernel2_L2);
          source(mm[i]) = "mm7_kernel2_L2.cc";
        }
