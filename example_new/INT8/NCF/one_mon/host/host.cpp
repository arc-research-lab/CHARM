...
int main(int argc, char** argv) {
    ...
    //layer6
    std::vector<std::vector<int16_t>>layer6_in0(layer[6][0],std::vector<int16_t>(layer[6][1],1));
    std::vector<std::vector<int16_t>>layer6_in1(layer[6][1],std::vector<int16_t>(layer[6][2],1));
    std::vector<std::vector<int16_t>>layer6_golden(layer[6][0],std::vector<int16_t>(layer[6][2],1));

    //layer7
    std::vector<std::vector<int16_t>>layer7_in0(layer[7][0],std::vector<int16_t>(layer[7][1],1));
    std::vector<std::vector<int16_t>>layer7_in1(layer[7][1],std::vector<int16_t>(layer[7][2],1));
    std::vector<std::vector<int16_t>>layer7_golden(layer[7][0],std::vector<int16_t>(layer[7][2],1));

    //layer8
    std::vector<std::vector<int16_t>>layer8_in0(layer[8][0],std::vector<int16_t>(layer[8][1],1));
    std::vector<std::vector<int16_t>>layer8_in1(layer[8][1],std::vector<int16_t>(layer[8][2],1));
    std::vector<std::vector<int16_t>>layer8_golden(layer[8][0],std::vector<int16_t>(layer[8][2],1));
    ...
    for (int m = 0; m < layer[6][0]; m++) {
        for (int k = 0; k < layer[6][1]; k++) {
            layer6_in0[m][k]= (int16_t)rand();
        }
    }
    srand (time(0));
    for (int k = 0; k < layer[6][1]; k++) {
        for (int n = 0; n < layer[6][2]; n++) {
            layer6_in1[k][n]= (int16_t)rand();
        }
    }
    ...
    for (int m = 0; m < layer[7][0]; m++) {
        for (int k = 0; k < layer[7][1]; k++) {
            layer7_in0[m][k]= (int16_t)rand();
        }
    }
    srand (time(0));
    for (int k = 0; k < layer[7][1]; k++) {
        for (int n = 0; n < layer[7][2]; n++) {
            layer7_in1[k][n]= (int16_t)rand();
        }
    }
    ...
    for (int m = 0; m < layer[8][0]; m++) {
        for (int k = 0; k < layer[8][1]; k++) {
            layer8_in0[m][k]= (int16_t)rand();
        }
    }
    srand (time(0));
    for (int k = 0; k < layer[8][1]; k++) {
        for (int n = 0; n < layer[8][2]; n++) {
            layer8_in1[k][n]= (int16_t)rand();
        }
    }
    ...
    //layer6
    auto bohdl_layer6_in0 = xrt::bo(device, (layer[6][0]*layer[6][1]) * sizeof(int16_t), dma0.group_id(0));
    auto bomapped_layer6_in0 = bohdl_layer6_in0.map<int16_t*>();
    auto bohdl_layer6_in1 = xrt::bo(device, (layer[6][1]*layer[6][2]) * sizeof(int16_t), dma0.group_id(0));
    auto bomapped_layer6_in1 = bohdl_layer6_in1.map<int16_t*>(); 

    //layer7
    auto bohdl_layer7_in0 = xrt::bo(device, (layer[7][0]*layer[7][1]) * sizeof(int16_t), dma0.group_id(0));
    auto bomapped_layer7_in0 = bohdl_layer7_in0.map<int16_t*>();
    auto bohdl_layer7_in1 = xrt::bo(device, (layer[7][1]*layer[7][2]) * sizeof(int16_t), dma0.group_id(0));
    auto bomapped_layer7_in1 = bohdl_layer7_in1.map<int16_t*>();

    //layer8
    auto bohdl_layer8_in0 = xrt::bo(device, (layer[8][0]*layer[8][1]) * sizeof(int16_t), dma0.group_id(0));
    auto bomapped_layer8_in0 = bohdl_layer8_in0.map<int16_t*>();
    auto bohdl_layer8_in1 = xrt::bo(device, (layer[8][1]*layer[8][2]) * sizeof(int16_t), dma0.group_id(0));
    auto bomapped_layer8_in1 = bohdl_layer8_in1.map<int16_t*>();
    ...
    //layer6
    bohdl_layer6_in0.sync(XCL_BO_SYNC_BO_TO_DEVICE , (layer[6][0]*layer[6][1])* sizeof(int16_t),0);
    bohdl_layer6_in1.sync(XCL_BO_SYNC_BO_TO_DEVICE , (layer[6][1]*layer[6][2])* sizeof(int16_t),0);

    //layer7
    bohdl_layer7_in0.sync(XCL_BO_SYNC_BO_TO_DEVICE , (layer[7][0]*layer[7][1])* sizeof(int16_t),0);
    bohdl_layer7_in1.sync(XCL_BO_SYNC_BO_TO_DEVICE , (layer[7][1]*layer[7][2])* sizeof(int16_t),0);

    //layer8
    bohdl_layer8_in0.sync(XCL_BO_SYNC_BO_TO_DEVICE , (layer[8][0]*layer[8][1])* sizeof(int16_t),0);
    bohdl_layer8_in1.sync(XCL_BO_SYNC_BO_TO_DEVICE , (layer[8][1]*layer[8][2])* sizeof(int16_t),0);
    ...
    //layer6
    auto bohdl_layer6_out = xrt::bo(device, (layer[6][0]*layer[6][2]) * sizeof(int16_t), dma0.group_id(0));
    auto bomapped_layer6_out = bohdl_layer6_out.map<int16_t*>();

    //layer7
    auto bohdl_layer7_out = xrt::bo(device, (layer[7][0]*layer[7][2]) * sizeof(int16_t), dma0.group_id(0));
    auto bomapped_layer7_out = bohdl_layer7_out.map<int16_t*>();

    //layer8
    auto bohdl_layer8_out = xrt::bo(device, (layer[8][0]*layer[8][2]) * sizeof(int16_t), dma0.group_id(0));
    auto bomapped_layer8_out = bohdl_layer8_out.map<int16_t*>();
    ...
    // start input kernels run handles
        ...
        dma_run = dma0(bohdl_layer4_in0, bohdl_layer4_in1,bohdl_layer4_out,
                nullptr, nullptr, nullptr, nullptr,
                nullptr, nullptr, nullptr, nullptr,
                nullptr, nullptr, nullptr, nullptr,
                nullptr, nullptr, nullptr, nullptr,
        ...
        dma_run = dma0(bohdl_layer7_in0, bohdl_layer7_in1,bohdl_layer7_out,
                nullptr, nullptr, nullptr, nullptr,
                nullptr, nullptr, nullptr, nullptr,
                nullptr, nullptr, nullptr, nullptr,
                nullptr, nullptr, nullptr, nullptr,
        ...
        dma_run = dma0(bohdl_layer8_in0, bohdl_layer8_in1,bohdl_layer8_out,
                ...
                nullptr, nullptr, nullptr, nullptr,
                nullptr, nullptr, nullptr, nullptr,
                (layer[8][0]/(M_ACC0)),(layer[8][1]/K_ACC0),(layer[8][2]/N_ACC0));
        dma_run.wait();
    ...
    bohdl_layer6_out.sync(XCL_BO_SYNC_BO_FROM_DEVICE, (layer[6][0]*layer[6][2])* sizeof(int16_t),/*OFFSET=*/ 0);
    bohdl_layer7_out.sync(XCL_BO_SYNC_BO_FROM_DEVICE, (layer[7][0]*layer[7][2])* sizeof(int16_t),/*OFFSET=*/ 0);
    bohdl_layer8_out.sync(XCL_BO_SYNC_BO_FROM_DEVICE, (layer[8][0]*layer[8][2])* sizeof(int16_t),/*OFFSET=*/ 0);
...

    {
        ...
        for (int m = 0; m < layer[6][0]; m++) {
            for (int n = 0; n < layer[6][2]; n++) {
                if(abs((int16_t)(bomapped_layer6_out[m+n*layer[6][0]])-layer6_golden[m][n])>=1e-3){
                    errorCount6++;
                }
            }
        }
        ...
        for (int m = 0; m < layer[7][0]; m++) {
            for (int n = 0; n < layer[7][2]; n++) {
                if(abs((int16_t)(bomapped_layer7_out[m+n*layer[7][0]])-layer7_golden[m][n])>=1e-3){
                    errorCount7++;
                }
            }
        }
        ...
        for (int m = 0; m < layer[8][0]; m++) {
            for (int n = 0; n < layer[8][2]; n++) {
                if(abs((int16_t)(bomapped_layer8_out[m+n*layer[8][0]])-layer8_golden[m][n])>=1e-3){
                    errorCount8++;
                }
            }
        }
