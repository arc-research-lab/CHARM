...
void mm5_kernel2_L2(input_window_int16* __restrict matA,
    ...
    window_incr(matA,L2_h1);                                
    buf_matA0=upd_w(buf_matA0,1,window_read_v16(matA));  
    window_incr(matA,L2_h1);

    buf_matB0 = upd_v(buf_matB0,0,window_read_v8(matB));  
    window_incr(matB,L2_w1);
    buf_matB0 = upd_v(buf_matB0,1,window_read_v8(matB));  
    window_decr(matB,L2_jump_B0);
    
    for (unsigned int i=0;i<L2_boundary_i;i++)  //i/16
    chess_prepare_for_pipelining
    chess_loop_range(L2_boundary_i,)
    {
        for (unsigned int j=0;j<L2_boundary_j;j++)  // j/2
        chess_prepare_for_pipelining
        chess_loop_range(L2_boundary_j,)
        {
