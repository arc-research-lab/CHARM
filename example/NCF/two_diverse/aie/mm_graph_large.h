#ifndef _MM_GRAPH_LARGE_H_
#define _MM_GRAPH_LARGE_H_
#include <adf.h>
#include "para.h"
using namespace adf;
...
private:
	adf::kernel mm_x4 [NUM_ENGINES_PER_PAC_LARGE];
	adf::pktsplit<NUM_ENGINES_PER_PAC_LARGE>  sp_a0;
	adf::pktsplit<NUM_ENGINES_PER_PAC_LARGE>  sp_b0;
...
public:
	...
	mm_x4_graph() {
		...
		sp_a0  = adf::pktsplit<NUM_ENGINES_PER_PAC_LARGE>::create();
		sp_b0  = adf::pktsplit<NUM_ENGINES_PER_PAC_LARGE>::create();
		...
		for (int row =0; row<NUM_ENGINES_PER_PAC_LARGE; row++)  {
			if(row==0){
		...
		for (int row =0; row<NUM_ENGINES_PER_PAC_LARGE; row++)  {
			...
			if(row<NUM_ENGINES_PER_PAC_LARGE-1){
