    ...
    for(ap_uint<64> tx=0;tx<TX;tx++){
        for(ap_uint<64> tz=0;tz<TZ;tz++){
            for(ap_uint<64> ty=0;ty<TY;ty++){
                for(ap_uint<64> k=0;k<K;k++){
                    for(ap_uint<64> m=0;m<(M/A_PER_TRA);m++){
                    #pragma HLS PIPELINE II = 1
                        ap_uint<64> pos=m+k*(M/A_PER_TRA)+ty*K*(M/A_PER_TRA)+tx*TY*K*(M/A_PER_TRA);
                        addrA_out.write(pos);
                    }
                }
            }
        ...
        for(ap_uint<64> tz=0;tz<TZ;tz++){
            for(ap_uint<64> ty=0;ty<TY;ty++){
                ...
                for(ap_uint<64> j=0;j<N;j++){
                    ...
                    #pragma HLS PIPELINE II = 1
                        pos=i+j*(K/A_PER_TRA)+ty*(K/A_PER_TRA)*N+tz*N*(K/A_PER_TRA)*TY;
                        addrB_out.write(pos);
                    }
                }
            }
...
#pragma HLS inline off
    for(ap_uint<64> tx=0;tx<TX;tx++){
        for(ap_uint<64> tz=0;tz<TZ;tz++){
            for(ap_uint<64> j=0;j<N;j++){
                for(ap_uint<64> i=0;i<M/C_PER_TRA;){
                #pragma HLS PIPELINE II = 1
                    ap_uint<64> pos=i+j*(M/C_PER_TRA)+tz*N*(M/C_PER_TRA)+tx*TZ*N*(M/C_PER_TRA);
                    addrC_out.write(pos);
                    i++;
                }
            }
