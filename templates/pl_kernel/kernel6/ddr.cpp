...
void loadA(axis_stream_A& dataA_in, ap_uint<BUFF_WIDTH> a_buf[A*(B/PACK_IN)][X*Y][LEFT_SIZE_BUFF],bool enable){
#pragma HLS inline off
    if(enable){
        for(int y=0;y<Y;y++){
            for(int b=0;b<B;b++){
                    ...
                    for(int x=0;x<X;x++){
                        for(int a=0;a<A;a++){
                            ...
                            #pragma HLS dependence variable=a_buf type=intra false
                                ...
                                a_buf[pos1][pos0][k]=temp_data(63,0);
                                a_buf[pos1][pos0][k+64]=temp_data(127,64);
                                a_buf[pos1][pos0][k+128]=temp_data(191,128);
                                a_buf[pos1][pos0][k+192]=temp_data(255,192);
                                a_buf[pos1][pos0][k+256]=temp_data(319,256);
                                a_buf[pos1][pos0][k+320]=temp_data(383,320);
                                a_buf[pos1][pos0][k+384]=temp_data(447,384);
                                a_buf[pos1][pos0][k+448]=temp_data(511,448);
                            }
                        }
                    }
                }
...
void loadB(axis_stream_B& dataB_in, ap_uint<PLIO_WIDTH> b_buf[(B/PACK_IN)*C][Y*Z][RIGHT_SIZE],bool enable){
#pragma HLS inline off
    if(enable){
        for(int z=0;z<Z;z++){
            for(int c=0;c<C;c++){
                    ...
                    for(int y=0;y<Y;y++){
                        for(int b=0;b<B;b++){
                            ...
                            #pragma HLS dependence variable=b_buf type=intra false
                                ...
                                b_buf[pos1][pos0][w2]=temp_data(127,0);
                                b_buf[pos1][pos0][w2+64]=temp_data(255,128);
                                b_buf[pos1][pos0][w2+128]=temp_data(383,256);
                                b_buf[pos1][pos0][w2+192]=temp_data(511,384);
                            }
                        }
                    }
                }
...
void storeC(axis_stream_C& dataC_out, ap_uint<BUFF_WIDTH> c_buf[A*C/PACK_OUT][2][X*Z][OUT_SIZE_BUFF/2], bool enable){
    #pragma HLS inline off
        if(enable){
            for(int z=0;z<Z;z++){
                for(int c=0;c<C;c++){
                        ...
                        for(int x=0;x<X;x++){
                            for (int a=0;a<A;a++){
                                ...
                                #pragma HLS PIPELINE II = 1
                                    ...
                                    temp_data(63,0)   =c_buf[pos2][pos1][pos0][w2/2];
                                    temp_data(127,64) =c_buf[pos2][pos1][pos0][w2/2+32 ];
                                    temp_data(191,128)=c_buf[pos2][pos1][pos0][w2/2+64 ];
                                    temp_data(255,192)=c_buf[pos2][pos1][pos0][w2/2+96 ];
                                    temp_data(319,256)=c_buf[pos2][pos1][pos0][w2/2+128];
                                    temp_data(383,320)=c_buf[pos2][pos1][pos0][w2/2+160];
                                    temp_data(447,384)=c_buf[pos2][pos1][pos0][w2/2+192];
                                    temp_data(511,448)=c_buf[pos2][pos1][pos0][w2/2+224];
            ...
            for(int x = 0; x < X*Z; x++){
                ...
                #pragma HLS dependence variable=c_buf type=intra false
                    for(int a = 0; a < (A*C/PACK_OUT); a++){
                        ...
                        c_buf[a][0][x][w2+64 ]=0; 
                        c_buf[a][0][x][w2+96 ]=0;
                        c_buf[a][0][x][w2+128]=0; 
                        c_buf[a][0][x][w2+160]=0;
                        c_buf[a][0][x][w2+192]=0; 
                        c_buf[a][0][x][w2+224]=0;
                        c_buf[a][1][x][w2]=0; 
                        c_buf[a][1][x][w2+32 ]=0;
                        c_buf[a][1][x][w2+64 ]=0; 
                        c_buf[a][1][x][w2+96 ]=0;
                        c_buf[a][1][x][w2+128]=0; 
                        c_buf[a][1][x][w2+160]=0;
                        c_buf[a][1][x][w2+192]=0; 
                        c_buf[a][1][x][w2+224]=0;
                    }
                }
            }
        }
