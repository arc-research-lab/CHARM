...
void compute(axis_stream_A& dataA_out, axis_stream_B& dataB_out, axis_stream_C& dataC_in,
    ...
    ap_uint<BUFF_WIDTH> buff0_A[A*(B/PACK_IN)][X*Y][LEFT_SIZE_BUFF];
    ...
    #pragma HLS ARRAY_PARTITION variable=buff0_A block factor=BUFFA_FACTOR dim=3
    
    ap_uint<BUFF_WIDTH> buff1_A[A*(B/PACK_IN)][X*Y][LEFT_SIZE_BUFF];
    ...
    #pragma HLS ARRAY_PARTITION variable=buff1_A block factor=BUFFA_FACTOR dim=3

    ap_uint<PLIO_WIDTH> buff0_B[(B/PACK_IN)*C][Z*Y][RIGHT_SIZE];
    ...
    #pragma HLS ARRAY_PARTITION variable=buff0_B block factor=BUFFB_FACTOR dim=3

    ap_uint<PLIO_WIDTH> buff1_B[(B/PACK_IN)*C][Z*Y][RIGHT_SIZE];
    ...
    #pragma HLS ARRAY_PARTITION variable=buff1_B block factor=BUFFB_FACTOR dim=3

    ap_uint<BUFF_WIDTH> buff0_C[(C/PACK_OUT)*A][2][Z*X][OUT_SIZE_BUFF/2];
    ...
    #pragma HLS ARRAY_PARTITION variable=buff0_C block factor=BUFFC_FACTOR dim=4

    ap_uint<BUFF_WIDTH> buff1_C[(C/PACK_OUT)*A][2][Z*X][OUT_SIZE_BUFF/2];
    ...
    #pragma HLS ARRAY_PARTITION variable=buff1_C block factor=BUFFC_FACTOR dim=4
